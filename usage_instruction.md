...
1. **Choose Patient**
   ...
   - The dropdown shows all available patients

2. **Load Patient Data**
   - Click "🔬 Load & Go to Segmentation →" button
   - The system will:
...
### Automatic Analysis Chain

After tracking completes, the system automatically:

1. **Converts to NIfTI**
   - Reconstructs 3D segmentation from 2D masks
   - Preserves original spatial information and headers

2. **Calculates Volume**
   - Counts segmented voxels
   - Computes volume in mm³ and mL
   - Saves volume report

3. **Runs Parcellation** (if available)
   - Identifies affected brain regions
   - Uses FreeSurfer atlas
   - Generates anatomical location report

4. **Generates AI Reports**
   - Creates clinical report for radiologists
   - Creates patient-friendly simplified report
   - Saves as DOCX files

5. **Updates Database**
   - Stores results in patient_results.json
   - Updates comman_format.json with latest findings
...
1. **Click "📄 Reports" Tab**
   - After tracking and analysis complete
   - System automatically switches to this tab

2. **View Clinical Report**
...
4. **Download Reports**
   ...
   - Files are ready for sharing or printing
   - Saved in patient folder for permanent storage
...
2. **View Volume Plot**
   - Line graph showing lesion volume over time
   - X-axis: Session dates
   - Y-axis: Volume in mL
   - Automatically updates when new sessions are analyzed

3. **Refresh Plot**
   - Click "🔄 Refresh Plot" to update with latest data

4. **Interpretation**
   - Increasing trend: Lesion growth
   - Decreasing trend: Lesion reduction/recovery
...
If automatic report generation needs to be rerun:

1. **Navigate to Reports Tab**
2. **Click "🤖 Generate MedGemma Reports"**
...
### Issue: "No video loaded" Error

**Problem**: Clicked segmentation button before loading video

**Solution**:
1. Go back and click "🎬 Use selected scan (NIfTI) as video"
2. Wait for processing to complete
3. Try segmentation again

### Issue: Segmentation Not Appearing

**Possible Causes:**
- No annotation drawn/clicked
- Wrong frame selected
- Model not loaded properly

**Solutions:**
1. Verify you've drawn on the canvas or added click points
2. Check "Current Frame" display shows correct frame number
3. Review console for error messages
4. Try different checkpoint or config

### Issue: Tracking Takes Too Long

**Solutions:**
1. Reduce frame rate (increase frame interval)
2. Use smaller model (e.g., tiny variant)
3. Process shorter video segments
4. Check GPU utilization is high (not CPU-bound)

### Issue: Reports Not Generating

**Check:**
1. HF_TOKEN environment variable is set correctly
2. Internet connection is active (MedGemma needs API access)
3. Segmentation completed successfully
4. Check console for specific error messages

**Solutions:**
- Restart application with proper HF_TOKEN
- Manually click "Generate MedGemma Reports" button
- Verify patient folder structure is correct

### Issue: GPU Out of Memory

**Solutions:**
1. Close other GPU-using applications
2. Use smaller model checkpoint
3. Reduce batch size (if in code)
4. Process fewer frames (lower frame rate)
5. Restart application to clear GPU memory

### Issue: Downloaded Files Not Opening

**Check:**
1. File actually downloaded (check Downloads folder)
2. File size is > 0 bytes
3. Correct application to open (DOCX needs Word/LibreOffice)

**Solutions:**
- Try download again
- Copy file path and open directly from file manager
- Check file permissions

### Issue: Volume Calculations Seem Wrong

**Verify:**
1. NIfTI header information is correct (check voxel dimensions)
2. Segmentation covers intended region
3. Units are as expected (mm³ vs mL)

**Solutions:**
- Validate source NIfTI file metadata
- Review segmentation visually
- Check volume report text file for details
...
- **Click**: Add point (in click mode)

### Efficient Review
- **Use Slider**: Quickly scan through frames
- **Frame Display**: Shows current frame number for reference
- **Status Messages**: Always read status/error messages
...
├── output_files/{session_id}/      # NIfTI exports, reports
