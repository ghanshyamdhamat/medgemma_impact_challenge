...
class MedGemmaSimplify:
    ...
    def generate_reports(self, mr_volume_path, slice_index, segmentation_mask_path, manual_report_context, 
        ...
        clinical_report = self.processor.batch_decode(
            generated_ids, 
            skip_special_tokens=True
        )[0].strip()
        print("Clinical Report Generated:\n", clinical_report)

        self.save_docx(clinical_report, clinical_docx, "🧠 Radiology Report")
        ...
        # Decode only the newly generated tokens (excluding the input prompt)
        generated_ids = output[:, inputs["input_ids"].shape[1]:]
        patient_report = self.processor.batch_decode(
            generated_ids, 
            skip_special_tokens=True
        )[0].strip()

        self.save_docx(patient_report, patient_docx, "🩺 Patient-Friendly Explanation")
